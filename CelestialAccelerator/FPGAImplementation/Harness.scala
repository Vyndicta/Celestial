...
/*
Must also add the CelestialNexysVideoConfig in another file.
*/
...
class NexysVideoHarness(override implicit val p: Parameters) extends NexysVideoShell {
  ...
  class HarnessLikeImpl extends Impl with HasHarnessInstantiators {
    ...
    val chiptop = instantiateChipTops().head
    val celestialPortsOpt = chiptop match {
      case celestial: HasCelestialTopIO => Some(celestial)
      case _ => None
    }

    // Debug: Print what we actually have
    println(s"ChipTop type: ${chiptop.getClass.getName}")
    println(s"Celestial ports found: ${celestialPortsOpt.isDefined}")
    ...
    val celestialLocked = celestialPortsOpt match {
      case Some(celestial) => 
        println("Found Celestial ports, using locked signal")
        celestial.io.locked
      case None => 
        println("No Celestial ports found, using false.B")
        false.B
    }
    
    // Synchronize the signal to the harness clock domain
    withClockAndReset(clk_100mhz, dutClock.in.head._1.reset) {
      val lockedSync = RegNext(RegNext(celestialLocked))
      other_leds(1) := lockedSync
    }
